...
use Closas\ShopBundle\Entity\Image;
use Closas\ShopBundle\Entity\Image\Size;
use Closas\ShopBundle\Entity\Content;
use Closas\ShopBundle\Entity\Content\Group;
...
class CmsController extends Controller {
    ...
    public function saveSiteAction(Request $request) {
        ...
        $reposCatLabel = $this->getDoctrine()->getRepository('ClosasShopBundle:Category\Label');
        ...
        $reposCat = $this->getDoctrine()->getRepository('ClosasShopBundle:Category');
        ...
        if ($delete_image) {
            ...
            if ($file) {
                ...
                $reposCatImage = $this->getDoctrine()->getRepository('ClosasShopBundle:Image');
                $reposCatImageSize = $this->getDoctrine()->getRepository('ClosasShopBundle:Image\Size');
    ...
    private function deleteImagesFile($field, $obj) {
        $reposImage = $this->getDoctrine()->getRepository('ClosasShopBundle:Image');
        $reposImageSize = $this->getDoctrine()->getRepository('ClosasShopBundle:Image\Size');
...
//        $arr_file1 = $request->files->all();
        ...
        foreach ($arr_content as $id => $data_content) {
            ...
            $reposContent = $this->getDoctrine()->getRepository('ClosasShopBundle:Content');
            ...
            foreach ($arr_obj_contents as $langid => $content) {
                ...
                if ($newImage && !$newContent) {
                    if ($category->getTyp()->getShortName() == 'BLOG') {
                        $contents = $this->getDoctrine()->getRepository('ClosasShopBundle:Content')->findBy(array('group' => $content->getGroup()));
    ...
    public function deleteContentAction(Request $request) {
        ...
        $catid = $request->request->get('catid');
        $langid = $request->request->get('langid');
        $content = $this->getDoctrine()->getRepository('ClosasShopBundle:Content')->findOneBy(array('id' => $id));

        $group = $content->getGroup();
        $contentGroup = $this->getDoctrine()->getRepository('ClosasShopBundle:Content\Group')->findOneBy(array('id' => $group->getId()));
        $contents = $this->getDoctrine()->getRepository('ClosasShopBundle:Content')->findBy(array('group' => $content->getGroup()));
        $em = $this->getDoctrine()->getManager();
        ...
        $category = $this->getDoctrine()->getRepository('ClosasShopBundle:Category')->findOneById($catid);
        $lang = $this->getDoctrine()->getRepository('ClosasShopBundle:Language')->findOneBy(array('short_name' => $langid));
    ...
    public function editSiteAction(Request $request) {
        ...
        $reposLanguage = $this->getDoctrine()->getRepository('ClosasShopBundle:Language');
        $languages = $reposLanguage->findAll();
        $reposCategoryLabel = $this->getDoctrine()->getRepository('ClosasShopBundle:Category\Label');
        foreach ($languages as $language) {
...
//
    ...
    private function getContent($category, $lang, $add_content = false) {
        ...
        $html = '';
        $id = $category->getId();
        $category = $this->getDoctrine()->getRepository('ClosasShopBundle:Category')->getAdminOrderContents($id);
        if (is_null($category)) {
            $category = $this->getDoctrine()->getRepository('ClosasShopBundle:Category')->findOneById($id);
        }
        ...
        // all url keys
        $arr_url_keys = array();
        $cats = $this->getDoctrine()->getRepository('ClosasShopBundle:Category')->findAll();
        foreach ($cats as $cat) {
    ...
    private function setContentUrlKey($category, $content, $em, $blog_url_key) {
        ...
        $group = $this->getDoctrine()->getRepository('ClosasShopBundle:Content\Group')->findOneById($groupid);
        if (!is_null($group->getUrlKey())) {
            return;
        }
        // set urlkey cor content if blog
        if ($category->getTyp()->getShortName() == 'BLOG') {
            $groups = $this->getDoctrine()->getRepository('ClosasShopBundle:Content\Group')->findAll();
            $isNotFound = FALSE;
            while (!$isNotFound) {
                $i = 0;
                $url_key_found = FALSE;
