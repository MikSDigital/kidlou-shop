...
use Closas\ShopBundle\Entity\Gift;
...
class GiftController extends Controller {
    ...
    public function listAction() {
        $gifts = $this->getDoctrine()->getRepository('ClosasShopBundle:Gift')->findAll();
        return array(
            'gifts' => $gifts
        );
    }
    ...
    public function newAction($id = null, Request $request) {
        if ($id != null) {
            $gift = $this->getDoctrine()->getRepository('ClosasShopBundle:Gift')->findOneById($id);
        } else {
            $gift = new Gift();
        }
        ...
        foreach ($langs as $lang) {
            if ($this->getDoctrine()->getRepository('ClosasShopBundle:Gift\Text')->findOneBy(array('gift' => $gift, 'lang' => $this->getDoctrine()->getRepository('ClosasShopBundle:Language')->findOneBy(array('short_name' => $lang->getShortName()))))) {
                $arr_lang[$lang->getShortName()] = $this->getDoctrine()->getRepository('ClosasShopBundle:Gift\Text')->findOneBy(array('gift' => $gift, 'lang' => $this->getDoctrine()->getRepository('ClosasShopBundle:Language')->findOneBy(array('short_name' => $lang->getShortName()))))->getDescription();
            } else {
        ...
        $form = $this->createFormBuilder($gift)
                ...
                ->add('codes', EntityType::class, array(
                    ...
                    'mapped' => false,
                    'required' => false,
                    'class' => 'Closas\ShopBundle\Entity\Gift\Coupon',
    ...
    private function setTexts($gift, $request) {
        ...
        $langs = $this->getDoctrine()->getRepository('ClosasShopBundle:Language')->findAll();
        foreach ($langs as $lang) {
            $text = $this->getDoctrine()->getRepository('ClosasShopBundle:Gift\Text')->findOneBy(array('lang' => $lang, 'gift' => $gift));
            if (!$text) {
                $text = new Text();
            }
            $text->setLang($lang);
    ...
    private function setCodes($gift) {
        $em = $this->getDoctrine()->getManager();
        $coupons = $this->getDoctrine()->getRepository('ClosasShopBundle:Gift\Coupon')->findBy(array('gift' => $gift));
        $count_codes = $gift->getNumberCodes() - count($coupons);
        for ($i = 0; $i < $count_codes; $i++) {
            $isCode = FALSE;
            while (!$isCode) {
                $code = $this->createCodes($gift->getLengthCode());
                $coupon = $this->getDoctrine()->getRepository('ClosasShopBundle:Gift\Coupon')->findOneBy(array('code' => $code));
                if (!$coupon) {
                    $isCode = TRUE;
                }
            }
