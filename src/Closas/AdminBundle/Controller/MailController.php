...
use Closas\ShopBundle\Entity\Inbox;
...
class MailController extends Controller {
    ...
    public function listAction() {
        $mails = $this->getDoctrine()->getRepository('ClosasShopBundle:Mail')->findAll();
        return array(
            'mails' => $mails
        );
    }
    ...
    public function detailAction($id = null, Request $request) {
        if ($id != null) {
            $reposMail = $this->getDoctrine()->getRepository('ClosasShopBundle:Mail');
    ...
    public function sendAction($id = null, Request $request) {
        if ($id != null) {
            $reposMail = $this->getDoctrine()->getRepository('ClosasShopBundle:Mail\Send');
            $mail = $reposMail->findOneById($id);
        } else {
            $mail = new Send();
        }
    ...
    public function sendlistAction() {
        $sends = $this->getDoctrine()->getRepository('ClosasShopBundle:Mail\Send')->findAll();
        return array(
            'sends' => $sends
        );
    }
    ...
    public function inboxlistAction() {
        ...
        foreach ($mailboxInfos as $mailboxInfo) {
            $inbox = $this->getDoctrine()->getRepository('ClosasShopBundle:Inbox')->findOneBy(array('email_id' => $mailboxInfo->uid));
            if (!$inbox) {
                $inbox = new Inbox();
            }
        ...
        $inboxs = $this->getDoctrine()->getRepository('ClosasShopBundle:Inbox')->findBy(array(), array('date' => 'DESC'));
    ...
    public function inboxAction($id) {
        $inbox = $this->getDoctrine()->getRepository('ClosasShopBundle:Inbox')->findOneById($id);
        if (!$inbox) {
            $inbox = new Inbox();
        }
        $mailbox = $this->_getMailbox();
    ...
    public function inboxdeleteAction(Request $request) {
        ...
        $inboxs = $this->getDoctrine()->getRepository('ClosasShopBundle:Inbox')->findBy(array('id' => $datas));
        $mailbox = $this->_getMailbox();
        $em = $this->getDoctrine()->getManager();
