...
use Closas\ShopBundle\Helper\Calendar\Day;
...
class Calendar {
    ...
    private function getDeliverStandard() {
        return $this->em->getRepository('ClosasShopBundle:Deliver\Standard')->findOneBy([]);
    }
    ...
    private function getDeliverException() {
        return $this->em->getRepository('ClosasShopBundle:Deliver')->findByMonthYear($this->getMonthCurrent(), $this->getYearCurrent(), $this->getMonthAfter(), $this->getYearAfter());
    }
    ...
    public function getCountDaysFromTo($product_id = '') {
        ...
        $product = $this->em->getRepository('ClosasShopBundle:Product')->findOneById($product_id);
        ...
        if ($date_from == '' && $date_to == '' && !$this->container->get('session')->get('quote_id')) {
            ...
            foreach ($additionals as $additional) {
                if ($additional) {
                    $arr_additionals[] = $this->em->getRepository('ClosasShopBundle:Product')->findOneById($additional);
                }
            }
        }
    ...
    private function getQuoteAdditionals($product_id) {
        ...
        if ($quote_id) {
            $quote = $this->em->getRepository('ClosasShopBundle:Quote')->findOneById($quote_id);
            $additionals = $this->em->getRepository('ClosasShopBundle:Map\QuoteProductAdditional')->findBy(array('quote' => $quote, 'parent' => $product_id));
            $arr_additionals = array();
            foreach ($additionals as $additional) {
                $arr_additionals[] = $this->em->getRepository('ClosasShopBundle:Product')->findOneById($additional->getChildren());
            }
    ...
    private function getIsRelationProduct($product_id, $quote_id) {
        ...
        if (strpos($product->getSku(), '_') !== FALSE) {
            ...
            $products = $this->em->getRepository('ClosasShopBundle:Product')->findRelationProducts($skus[0], $product->getSku());
    ...
    private function getCurrentQuotes($quote_id) {
        return $this->em->getRepository('ClosasShopBundle:Quote')->getCurrentQuotes($quote_id);
    }
    ...
    private function getCurrentOrders($product_id) {
        $statutes = $this->em->getRepository('ClosasShopBundle:Order\Status')->findAll();
        $arr_status = array();
        ...
        $product = $this->em->getRepository('ClosasShopBundle:Product')->findOneById($product_id);
        return $this->em->getRepository('ClosasShopBundle:Order')->getOrderDates($product, $this->getMonthCurrent(), $this->getYearCurrent(), $this->getCountDays(), $arr_status);
    }
