...
use Closas\ShopBundle\Helper\Navigation;
...
class Common {
    ...
    public function getLanguage() {
        return $this->em->getRepository('ClosasShopBundle:Language')
    ...
    public function getLanguageAsArray() {
        $langs = $this->em->getRepository('ClosasShopBundle:Language')->findAll();
        $arr_lang = array();
    ...
    private function setZone() {
        $zones = $this->em->getRepository('ClosasShopBundle:Zone')->findAll();
    ...
    private function setCaution() {
        $cautions = $this->em->getRepository('ClosasShopBundle:Caution')->findAll();
    ...
    private function setShippings() {
        $this->shippings = $this->em->getRepository('ClosasShopBundle:Shipping')->findBy(array(), array('price_limit' => 'ASC'));
    }
    ...
    public function getGeodataPlzCity() {
        ...
        $reposPostalcode = $this->em->getRepository('ClosasShopBundle:Postalcode');
    ...
    public function getNavigation($navigation = '', $is = true, $isFrontend = false) {
        ...
        if (!$isFrontend) {
            $this->navcollection = $this->em
                    ->getRepository('ClosasShopBundle:Category')
                    ->findAll();
        } else {
            $this->navcollection = $this->em
                    ->getRepository('ClosasShopBundle:Category')
        ...
        $this->navlabelcollection = $this->em
                ->getRepository('ClosasShopBundle:Category\Label')
...
// has submenu
    ...
    public function getNivoSliderItem() {
        $slideLists = $this->em->getRepository('ClosasShopBundle:Nivoslider')->findBy(array('status' => TRUE));
        $arr_slider_ids = array();
        foreach ($slideLists as $slide) {
            $arr_slider_ids[] = $slide->getId();
        }
        $slide = $this->em
                ->getRepository('ClosasShopBundle:Nivoslider\Item')
                ->findAllByArray($this->request->getLocale(), $arr_slider_ids);

        $nivoConfig = $this->em
                ->getRepository('ClosasShopBundle:Nivoslider\Configuration')
                ->findFirst();
...
// suchen nach allen level + 1, das sind die hauptkategorien
        ...
        foreach ($arr_main_category as $main_catid => $category) {
            $this->arr_cat['maincategories'][$main_catid] = $this->em->getRepository('ClosasShopBundle:Category')->findOneById($main_catid);
        }
...
// create categories
        foreach ($arr_group_cats as $main_catid => $arr_category) {
            foreach ($arr_category as $categoryid) {
                $this->arr_cat['categories'][$main_catid][] = $this->em->getRepository('ClosasShopBundle:Category')->findOneById($categoryid);
            }
        }
        return $this;
    }
...
// has submenu
    ...
    public function getCategoryTyp($optionval = '') {
        $catTyps = $this->em->getRepository('ClosasShopBundle:Category\Typ')
                ->findAll();
...
// optionval undefined
    ...
    private function _getBasket() {
        ...
        $quote = $this->em->getRepository('ClosasShopBundle:Quote')->findOneById($quote_id);
        $basketsByQuote = $this->em->getRepository('ClosasShopBundle:Calendar')->getBasketsByQuote($quote);
        $arr_basket = array();
        ...
        foreach ($arr_basket as $product_id => $baskets) {
            foreach ($baskets as $calendar_id => $basket) {
                $obj_basket[$product_id]['product'] = $this->em->getRepository('ClosasShopBundle:Product')->findOneById($product_id);
                $obj_basket[$product_id]['calendar'][] = $this->em->getRepository('ClosasShopBundle:Calendar')->findOneById($calendar_id);
                if (isset($basket['children'])) {
                    foreach ($basket['children'] as $children) {
                        $obj_basket[$product_id]['children'][$children] = $this->em->getRepository('ClosasShopBundle:Product')->findOneById($children);
                    }
                }
            }
        }
    ...
    public function getCountByBasket() {
        ...
        $quote = $this->em->getRepository('ClosasShopBundle:Quote')->findOneById($quote_id);
        $objBasket = $this->em->getRepository('ClosasShopBundle:Calendar')->getCountByBasket($quote);
    ...
    public function setBasket() {
        ...
        $this->arr_basket_items = $this->em->getRepository('ClosasShopBundle:Quote')->getBasketItems($quote_id, 'image80', 'image80', $this->getRequest()->getLocale());
        $price = 0;
    ...
    public function sendCommonEmailMessage($subject, $text, $to_email) {
        $mail = $this->em->getRepository('ClosasShopBundle:Mail')->findOneBy(array('status' => TRUE, 'type' => 'common'));
        $arr_bcc_mails = '';
        if ($mail->getBccEmail() != '') {
