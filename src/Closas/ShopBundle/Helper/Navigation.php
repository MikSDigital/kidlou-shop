...
class Navigation {
    ...
    public function setProductPathFromUrl($url_key = '', $category = '') {
        if ($url_key != '') {
            ...
            $product = $this->em->getRepository('ClosasShopBundle:Product')->findOneBy(array('url_key' => $url_key));
            if ($product) {
                foreach ($product->getCategories() as $category) {
                    $this->setProductPathFromUrl('', $category);
                }
            } else {
                $category = $this->em->getRepository('ClosasShopBundle:Category')->findOneBy(array('url_key' => $url_key));
                $this->setProductPathFromUrl('', $category);
            }
        } else {
            if ($category->getLevel() > 1) {
                $this->arr_url_keys[] = $category->getUrlKey();
                $category = $this->em->getRepository('ClosasShopBundle:Category')->findOneBy(array('id' => $category->getParentId()));
                $this->setProductPathFromUrl('', $category);
            }
        }
        return $this;
    ...
    public function showCurrentTitle() {
        ...
        $category = $this->em->getRepository('ClosasShopBundle:Category')->findOneBy(array('url_key' => $temp_arr_urls[(count($temp_arr_urls) - 1)]));
    ...
    public function setMenu() {
        ...
        $navigations = $this->em->getRepository('ClosasShopBundle:Category')->getNavigation('PROD', 2);
        $this->setMenuProducts($navigations);
        $navigations = $this->em->getRepository('ClosasShopBundle:Category')->getNavigation('CMS', 1);
        $this->setMenuCms($navigations);
        $navigations = $this->em->getRepository('ClosasShopBundle:Category')->getNavigation('BLOG', 1);
    ...
    private function setMenuProducts($navigations, $url_key = '') {
        ...
        foreach ($navigations as $navigation) {
            ...
            $_navigations = $this->em->getRepository('ClosasShopBundle:Category')->findBy(array('parent_id' => $navigation->getId(), 'status' => 1));
...
//        return $html;
    ...
    public function setMainMenuProduct() {
        $navigations = $this->em->getRepository('ClosasShopBundle:Category')->getNavigation('PROD', 1);
        foreach ($navigations as $navigation) {
