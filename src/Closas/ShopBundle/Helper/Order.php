...
use Closas\ShopBundle\Entity\Order as EntityOrder;
use Closas\ShopBundle\Entity\Order\Item;
use Closas\ShopBundle\Entity\Order\Address as OrderAddress;
use Closas\ShopBundle\Entity\Order\Payment as OrderPayment;
use Closas\ShopBundle\Entity\Map\OrderProductAdditional;
use Closas\ShopBundle\Entity\Map\OrderItemAdditional;
use Closas\ShopBundle\Helper\Payment\Typ\Post;
use Closas\ShopBundle\Helper\Payment\Typ\Paypal;
...
//$this->request->getLocale()
    ...
    public function getPaymentName() {
        ...
        return $this->getEm()->getRepository('ClosasShopBundle:Order\Payment')->findOneBy(array('order' => $order))->getPaymentName();
    }
    ...
    private function getCurrentQuote() {
        ...
        return $this->getEm()->getRepository('ClosasShopBundle:Quote')->findOneById($quote_id);
    }
    ...
    public function getCurrentOrder() {
        ...
        return $this->getEm()->getRepository('ClosasShopBundle:Order')->findOneById($order_id);
    }
    ...
    private function getStatusByName($name) {
        return $this->getEm()->getRepository('ClosasShopBundle:Order\Status')->findOneBy(array('name' => $name));
    }
    ...
    public function getAllStatus() {
        return $this->getEm()->getRepository('ClosasShopBundle:Order\Status')->findAll();
    }
    ...
    private function _getOrderNumber() {
        ...
        $order = $this->getEm()->getRepository('ClosasShopBundle:Order')
    ...
    private function saveOrder() {
        $lang = $this->getEm()->getRepository('ClosasShopBundle:Language')->findOneBy(array('short_name' => $this->getRequest()->getLocale()));
        $order = $this->getEm()->getRepository('ClosasShopBundle:Order')->findOneBy(array('quote' => $this->getCurrentQuote()));
    ...
    private function saveOrderAddress() {
        $addressTyp = $this->getEm()->getRepository('ClosasShopBundle:Order\Address')->findOneBy(array('order' => $this->getCurrentOrder(), 'address_typ' => $this->getBillingType()));
// save billing
        if (!$addressTyp) {
...
// save shipping
        $addressTyp = $this->getEm()->getRepository('ClosasShopBundle:Order\Address')->findOneBy(array('order' => $this->getCurrentOrder(), 'address_typ' => $this->getShippingType()));
        if (!$addressTyp) {
            $addressTyp = new OrderAddress();
            $addressTyp->setCreated($this->getNowTime());
            $addressTyp->setOrder($this->getCurrentOrder());
    ...
    private function saveOrderPayment() {
        ...
        $paymenttyp = $this->getEm()->getRepository('ClosasShopBundle:Payment')->findOneById($paymenttyp_id);
        $paymentAdditional = json_encode($this->getInstitutPaymentAsArray($paymenttyp->getName()));
        $orderPayment = $this->getEm()->getRepository('ClosasShopBundle:Order\Payment')->findOneBy(array('order' => $this->getCurrentOrder()));
        if (!$orderPayment) {
    ...
    private function saveOrderCalendar() {
        $order = $this->getCurrentOrder();
        $calendars = $this->getEm()->getRepository('ClosasShopBundle:Calendar')->findBy(array('quote' => $this->getCurrentQuote()));
    ...
    private function saveOrderAdditional() {
        $order = $this->getCurrentOrder();
        $quoteAdditionals = $this->getEm()->getRepository('ClosasShopBundle:Map\QuoteProductAdditional')->findBy(array('quote' => $this->getCurrentQuote()));

        // delete order map if exists
        $orderAdditionals = $this->getEm()->getRepository('ClosasShopBundle:Map\OrderProductAdditional')
    ...
    public function saveOrderItem() {
        $order = $this->getCurrentOrder();
        $calendars = $this->getEm()->getRepository('ClosasShopBundle:Calendar')
                ->findBy(array('order' => $order));
        // check if order item exist if yes then remove
        $arr_order_item_additionals = array();
        foreach ($calendars as $calendar) {
            $items = $this->getEm()->getRepository('ClosasShopBundle:Order\Item')
        ...
        foreach ($arr_order_item_additionals as $item_additional) {
            $orderItemAdditionals = $this->getEm()->getRepository('ClosasShopBundle:Map\OrderItemAdditional')->findBy(array('parent' => $item_additional->getId()));
...
// add order to calendar data
        foreach ($calendars as $calendar) {
            ...
            $quoteAdditionals = $this->getEm()->getRepository('ClosasShopBundle:Map\QuoteProductAdditional')
                    ->findBy(array('quote' => $this->getCurrentQuote(), 'parent' => $calendar->getProduct()->getId()));
            foreach ($quoteAdditionals as $quoteAdditional) {
                $item = new Item();
                $item->setCalendar($calendar);
                $children = $this->getEm()->getRepository('ClosasShopBundle:Product')->findOneById($quoteAdditional->getChildren());
    ...
    public function getInstitutPaymentAsArray($name) {
        return $this->getEm()
                        ->getRepository('ClosasShopBundle:Payment\\' . $name)
    ...
    public function getPaymenttyp() {
        return $this->getEm()->getRepository('ClosasShopBundle:Order\Payment')->findOneBy(array('order' => $this->getCurrentOrder()));
    }
...
// check if url is intern, if has protocol then external link, if no protocol then internal link
    ...
    public function setOrderStatus($statustyp) {
        $order = $this->getEm()->getRepository('ClosasShopBundle:Order')->findOneById($this->getCurrentOrder()->getId());
        if ($order) {
    ...
    public function getName() {
        $orderAddress = $this->getEm()->getRepository('ClosasShopBundle:Order\Address')->findOneBy(array('order' => $this->getCurrentOrder(), 'address_typ' => $this->getBillingType()));
    ...
    public function getOrderNumber() {
        $order = $this->getEm()->getRepository('ClosasShopBundle:Order')->findOneBy(array('order' => $this->getCurrentOrder()));
    ...
    public function getOrderData($product) {
        return $this->getEm()->getRepository('ClosasShopBundle:Calendar')->getReservedDates($product
    ...
    public function getOrderDataById($id) {
        $arr_order = $this->getEm()->getRepository('ClosasShopBundle:Order')->getOrderData($id);
//        print_r($arr_order);
//        exit;
