...
class ImageResizer {
     ...
     */
    public function resizeImage($image, $newPath, $filename, $height = 0, $width = 0) {
        ...
        switch ($fileExtention) {
            case 'png' : case 'PNG' :
                ...
                break;
            case 'jpeg': case 'JPEG': case 'jpg': case 'JPG': $gd_image_src = imagecreatefromjpeg($image);
                break;
            case 'gif' : case 'GIF' : $gd_image_src = imagecreatefromgif($image);
                break;
            default: break;
        }
        ...
        $newFileName = $newPath . $filename . "." . $fileExtention;

        switch ($fileExtention) {
            case 'png' : case 'PNG' : imagepng($gd_image_dest, $newFileName);
                break;
            case 'jpeg' : case 'JPEG' : case 'jpg' : case 'JPG' : imagejpeg($gd_image_dest, $newFileName, $jpegQuality);
                break;
            case 'gif' : case 'GIF' : imagegif($gd_image_dest, $newFileName);
                break;
            default: break;
        }
        return $newPath;
